...
import { ArrowLeft, Clock, TrendingUp, Calendar, Lock, Save, Loader2, CheckCircle2, KeyRound } from 'lucide-react';
...
export default function StudentDetailPage({ params }: { params: { id: string } }) {
  ...
  const handlePasswordReset = async () => {
    ...
    setPwSaving(true);
    setPwSaved(false);
    const password_hash = await hashPassword(resetPw.trim());
    await supabase.from('profiles').update({ password_hash }).eq('id', params.id);
    setResetPw('');
    setPwSaving(false);
    setPwSaved(true);
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col justify-center items-center px-6">
        <div className="w-full max-w-md text-center">
          ...
          <p className="text-slate-500 text-sm mt-2 font-medium">先に管理画面で合い言葉を入力してください。</p>
          <Link href="/admin" className="inline-block mt-6 bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold hover:bg-indigo-700 transition-all">
            管理画面へ
          </Link>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 mb-6">
            ...
            <div className="mt-6 pt-6 border-t border-gray-100">
              <p className="text-sm font-black text-gray-800 flex items-center gap-2 mb-1">
                <KeyRound size={16} className="text-indigo-500" /> パスワードを設定 / リセット
              </p>
              <p className="text-xs text-gray-400 font-medium mb-3">新しいパスワードを入力して保存すると、この利用者のログインパスワードを上書きします。</p>
              <div className="flex flex-col sm:flex-row gap-3">
                <input
                  ...
                  className="flex-1 border border-gray-200 rounded-2xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-400 transition-all"
                />
                <button
                  onClick={handlePasswordReset}
                  disabled={pwSaving || !resetPw.trim()}
                ...
                >
                  {pwSaving ? <Loader2 size={16} className="animate-spin" />
                    : pwSaved ? <><CheckCircle2 size={16} /> 設定しました</>
                    : <><Save size={16} /> 設定する</>}
                </button>
              </div>
            </div>
          ...
          {activeTab === 'survey' && (
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 space-y-8">
              ...
              {/* ⑥ プログラム終了後の目標 */}
              <SurveyField label="プログラム終了後（6ヶ月後）の目標">
                <textarea
